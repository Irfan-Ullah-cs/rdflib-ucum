...
1. ``register_datatypes()`` calls ``rdflib.term.bind()`` for all 35 CDT types.
...
3. ``register_sparql_functions()`` adds utility functions:
   ``cdt:convert``, ``cdt:toSI``, ``cdt:getValue``, ``cdt:getUnit``,
   ``cdt:sameDimension``.
"""
...
# Public API
from .namespace import CDT, CDT_NS, is_cdt_datatype, ALL_CDT_TYPES, ALL_QUANTITY_TYPES
...
__all__ = [
    # Namespace
    "CDT", "CDT_NS", "is_cdt_datatype", "ALL_CDT_TYPES", "ALL_QUANTITY_TYPES",
    # Value types
